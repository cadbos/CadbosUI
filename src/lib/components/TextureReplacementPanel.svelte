...
<script lang="ts">
	...
	beforeNavigate(({ to }) => {
		if (submitting && !to?.route.id?.startsWith('/texture-replacement')) {
	...
	async function pollJob(id: string, signal: AbortSignal, run: number): Promise<void> {
		...
		while (!signal.aborted && run === pollRun) {
			...
			try {
				response = await fetch(`/api/texture-replacement/${encodeURIComponent(id)}`, { signal });
			} catch (error) {
	...
	async function submit(): Promise<void> {
		...
		try {
			...
			if (navigatedAwayWhileSubmitting || !page.route.id?.startsWith('/texture-replacement')) {
				return;
			}
			try {
				await goto(buildShareUrl('textureReplacement', request), {
					replaceState: true,
					keepFocus: true,
					noScroll: true
				});
...
<section class="step-card">
	<div class="step-header">
		...
		<h2>{t('textureReplacement.images')}</h2>
	</div>

	<div class="image-grid">
		<div class="image-column">
			<div class="column-header">
				<h3>{t('textureReplacement.sourceImage')}</h3>
		...
		<div class="image-column">
			<div class="column-header">
				<h3>{t('textureReplacement.referenceImage')}</h3>
				<span class="required-badge">{t('textureReplacement.required')}</span>
			</div>
			<ImageUpload
...
<section class="step-card">
	<div class="step-header">
		<span class="step-num" aria-hidden="true">②</span>
		<h2>{t('textureReplacement.surfaceLabel')}</h2>
		<span class="required-badge">{t('textureReplacement.required')}</span>
	</div>
...
<section class="step-card generate-section">
	<div class="step-header">
		<span class="step-num" aria-hidden="true">③</span>
		<h2>{t('textureReplacement.controls')}</h2>
	</div>
