...
>
	<button
		...
		onclick={() => (open = !open)}
	>
		<span class="code">{currency.displayCode.toUpperCase()}</span>
		<ChevronDown class="chevron" size={18} strokeWidth={1.8} aria-hidden="true" />
	</button>
	<div id="currency-menu" class="menu" hidden={!open}>
...
<style>
	...
	.currency-trigger :global(.chevron) {
	...
	.currency-trigger[aria-expanded='true'] :global(.chevron) {
