...
// only exists to consume it — including its size/megapixel nodes (62, 63) and
// scaled copy (4) — so the graph stays valid with a single input image. Node 28
// is the history-stitch preview only (not read by the app — only node 17's
// output is fetched) but is rewired too for correctness.
function omitReferenceImage(workflow: ComfyWorkflow): void {
	const referenceNode = workflow['2'];
	const referenceSizeNode = workflow['62'];
	const referenceMegapixelsNode = workflow['63'];
	...
	if (
		...
		!referenceSizeNode ||
		referenceSizeNode.class_type !== 'GetImageSize' ||
		!referenceMegapixelsNode ||
		referenceMegapixelsNode.class_type !== 'MathExpression|pysssss' ||
		!scaledReferenceNode ||
