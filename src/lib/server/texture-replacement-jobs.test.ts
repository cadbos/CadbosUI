...
describe('texture replacement jobs', () => {
	it('clamps completion spending at zero and warns', async () => {
		...
		const generation = await db
			.prepare(
				'SELECT -entry.amount AS amount_units, job.balance_after FROM generations generation ' +
			...
			.bind('job-1')
			.first<{ amount_units: number; balance_after: number }>();
		expect(credit?.balance).toBe(0);
		expect(generation).toEqual({ amount_units: 200, balance_after: 0 });
