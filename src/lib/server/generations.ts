...
// One row per paid generation (migrations/0011): the resulting image, the
...
async function finalizeGenerationOperationOnce(
		...
		db
			.prepare(
				'INSERT INTO ledger_transactions (id, occurred_at) SELECT ?, ? ' +
					"WHERE EXISTS (SELECT 1 FROM ledger_accounts WHERE user_id = ? AND kind = 'user_balance') " +
					"AND EXISTS (SELECT 1 FROM ledger_accounts WHERE asset = 'archai_token' AND kind = 'asset_balance')"
			)
	...
	statements.push(
		...
		db
			...
			.bind(row.id, row.output_url, row.input_url, row.input_hash),
		db
			.prepare(
...
export async function getGeneratedImageForUser(
	...
	const row = await db
		.prepare(
			'SELECT generation.id, generation.user_id, detail.output_url AS url, ' +
				'detail.input_url AS source_url, generation.kind, generation.created_at ' +
...
export async function findGenerationSourceByHash(
	...
	const row = await db
		.prepare(
			'SELECT detail.input_url AS source_url FROM generations generation ' +
				'JOIN image_generation_details detail ON detail.generation_id = generation.id ' +
				'WHERE generation.user_id = ? AND detail.input_hash = ? ' +
...
// /resources — not every input_url a generation ever recorded. An input URL
// only represents a real upload when input_hash is non-empty:
...
export async function listDistinctSourceImages(
	...
	const result = await db
		.prepare(
			'SELECT detail.input_url AS source_url, MAX(generation.created_at) AS created_at ' +
				'FROM generations generation JOIN image_generation_details detail ' +
				'ON detail.generation_id = generation.id ' +
				"WHERE generation.user_id = ? AND detail.input_hash != '' " +
				'GROUP BY detail.input_url ' +
	...
	const result = await db
		.prepare(
			'WITH credit_accounts AS (' +
				...
				"AND entry.account_id = (SELECT id FROM ledger_accounts WHERE asset = 'archai_token' AND kind = 'asset_balance') " +
