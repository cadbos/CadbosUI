...
async function loadProjectDetail(db: Database, projectRow: ProjectRow): Promise<ProjectDetail> {
	...
	for (let offset = 0; offset < sessionRows.length; offset += D1_MAX_BOUND_PARAMS) {
		...
		const generationRows = await db.all<SessionGenerationRow & { session_id: string }>(
			sql`SELECT g.id, g.session_id, g.result_media_id, g.source_media_id,
...
export async function getShareGenerationDetail(
	...
	const row = await db.get<ShareGenerationDetailRow>(
		sql`SELECT g.id, g.prompt, g.kind, g.created_at, g.form_snapshot FROM generations g
			JOIN project_sessions ps ON ps.id = g.session_id
			JOIN projects p ON p.id = ps.project_id
			JOIN project_shares s ON s.project_id = p.id
			WHERE g.id = ${generationId} AND s.token = ${token} AND s.revoked_at IS NULL
