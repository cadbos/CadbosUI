...
const DEFAULT_FLUX_KONTEXT_EDIT_COST = 0.03;
const COMFYUI_REQUEST_TIMEOUT_MS = 120_000;
export const FLUX_KONTEXT_EDIT_TIMEOUT_MS = 10 * 60_000;
...
export function fluxKontextEditCost(platform: App.Platform | undefined): number {
	const configured = platform?.env?.FLUX_KONTEXT_EDIT_COST?.trim();
	if (!configured) return DEFAULT_FLUX_KONTEXT_EDIT_COST;
...
export async function cancelFluxKontextEdit(
...
export async function submitFluxKontextEdit(
	platform: App.Platform | undefined,
	request: { image: string; prompt: string },
	...
	const queued = await queueFluxKontextEdit(client, {
		clientId: jobId,
...
export async function pollFluxKontextEdit(
	platform: App.Platform | undefined,
	promptId: string
): Promise<ComfyDownloadedImage | null> {
	return getFluxKontextEditResult(
