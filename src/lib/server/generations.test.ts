...
function seedInvalidKindGeneration(
	database: D1Database,
	id: string,
	userId: string,
	createdAt: number
	...
	const amountUnits = toLedgerAmountUnits(1);
	database
	...
	database
		...
		.bind(transactionId, -amountUnits, userId)
		.run();
	database
		.prepare('INSERT INTO ledger_entries (transaction_id, account_id, amount) VALUES (?, ?, ?)')
		.bind(transactionId, 'app-credit:system', amountUnits)
		.run();
	database
		.prepare('INSERT INTO ledger_entries (transaction_id, account_id, amount) VALUES (?, ?, ?)')
		.bind(transactionId, 'archai-token', -amountUnits)
		.run();
	database
		.prepare('INSERT INTO ledger_entries (transaction_id, account_id, amount) VALUES (?, ?, ?)')
		.bind(transactionId, 'archai-token:system', amountUnits)
		.run();
	database
		.prepare(
	...
	database
		...
		.bind(id, `https://cdn.example.test/${id}.webp`, 'https://cdn.example.test/source.jpg', '')
		.run();
	database
...
describe('generation operations', () => {
	it('persists pending request attribution before confirmation and charges only confirmed work', async () => {
		...
		const operationId = await createGenerationOperation(db, 'user-1', {
			...
			sourceHash: generationInput.sourceHash,
...
describe('listCreditHistory', () => {
	...
	it('rejects an invalid stored generation kind', async () => {
		seedUser(db, 'user-1', 'pubkey-1');
		grantGenerationAccess(db, 'user-1', 5);
		seedInvalidKindGeneration(db, 'invalid-kind', 'user-1', 1000);
...
describe('findGenerationSourceByHash', () => {
	...
	it('never matches an empty hash, even against pre-migration rows', async () => {
		seedUser(db, 'user-1', 'pubkey-1');
		seedGeneratedImage(db, 'legacy', 'user-1', 1000);
