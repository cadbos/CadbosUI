...
const DEFAULT_PRO_MODE_COST = 0.03;
const COMFYUI_REQUEST_TIMEOUT_MS = 120_000;
export const PRO_MODE_TIMEOUT_MS = 10 * 60_000;
...
export function proModeCost(platform: App.Platform | undefined): number {
	const configured = platform?.env?.PRO_MODE_COST?.trim();
	if (!configured) return DEFAULT_PRO_MODE_COST;
	const cost = Number(configured);
	if (!Number.isFinite(cost) || cost <= 0) {
		throw new ComfyUiError('invalid_configuration', 'configuration', 'Invalid pro mode cost');
	}
	return cost;
}

export async function cancelProMode(
...
export async function submitProMode(
	platform: App.Platform | undefined,
	request: {
		image: string;
	...
	const queued = await queueProMode(client, {
		clientId: jobId,
...
export async function pollProMode(
	platform: App.Platform | undefined,
	promptId: string
): Promise<ComfyDownloadedImage | null> {
	return getProModeResult(
