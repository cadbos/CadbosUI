...
export async function createObjectReplacementJob(
	...
	await db.run(
		sql`INSERT INTO object_replacement_jobs
			...
			VALUES (${input.id}, ${input.userId}, ${input.comfyPromptId}, ${input.sceneMediaId}, ${input.sessionId}, ${input.referenceMediaId}, ${input.replacementObject}, ${input.cost}, 'processing', ${input.createdAt}, ${input.createdAt}, ${input.uploadQueueSec}, ${input.formSnapshot ? JSON.stringify(input.formSnapshot) : null})`
	);
...
export async function failObjectReplacementJob(
	...
	await db.run(
		sql`UPDATE object_replacement_jobs SET status = 'failed', error_code = ${errorCode}, updated_at = ${completedAt},
