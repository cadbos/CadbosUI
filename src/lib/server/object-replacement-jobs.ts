...
export async function createObjectReplacementJob(
	...
	await db.run(
		sql`INSERT INTO object_replacement_jobs
			...
			VALUES (${input.id}, ${input.userId}, ${input.comfyPromptId}, ${scene.id}, ${input.sessionId}, ${reference.id}, ${input.replacementObject}, ${input.cost}, 'processing', ${input.createdAt}, ${input.createdAt})`
	);
	...
	const [snapshotRows] = await db.batch([
		...
		db.run(
			sql`INSERT INTO generations
				...
				SELECT j.id, j.user_id, ${output.id}, j.scene_media_id, j.replacement_object, 'object-replacement', j.cost, c.balance, ${completedAt}, j.session_id
				FROM object_replacement_jobs j JOIN credits c ON c.user_id = j.user_id
				WHERE j.id = ${id} AND j.user_id = ${userId} AND j.status = 'processing'`
		),
		db.run(
			sql`UPDATE object_replacement_jobs SET status = 'completed', output_media_id = ${output.id},
