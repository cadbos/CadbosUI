...
import { apiError, parseBody, proModeRequestSchema } from '$lib/server/api';
...
import { cancelProMode, proModeCost, submitProMode } from '$lib/server/pro-mode';
import { createProModeJob } from '$lib/server/pro-mode-jobs';
...
const PRO_MODE_RATE_LIMIT = { windowMs: 60_000, max: 10 } as const;
const proModeInFlight = new Set<string>();
...
export const POST: RequestHandler = async ({ request, platform, locals, url }) => {
	...
	const parsed = await parseBody(request, proModeRequestSchema);
	...
	// that window (mirrors object-replacement's guard).
	const pubkey = locals.user.pubkey;
	if (proModeInFlight.has(pubkey)) {
		logRejection(409, 'request_in_progress');
		return apiError(409, 'request_in_progress', 'Pro mode request already in progress');
	}
	proModeInFlight.add(pubkey);
	...
	try {
		...
		const limited = await touchRateLimit(db, `pro-mode:${pubkey}`, Date.now(), PRO_MODE_RATE_LIMIT);
		if (limited) {
		...
		const media = await providerMediaBatch(db, platform, [
			...
			...(parsed.data.referenceImageKey ? [parsed.data.referenceImageKey] : [])
		]);
		if (!media) return apiError(404, 'image_not_found', 'Image not found');
		const sceneMedia = media.get(parsed.data.imageKey)!.media;
		const referenceMedia = parsed.data.referenceImageKey
			? media.get(parsed.data.referenceImageKey)!.media
			: undefined;

		let cost: number;
		try {
			cost = proModeCost(platform);
			...
			logFailure(500, 'pro_mode_failed', { operation: 'billing_precheck' });
			return apiError(500, 'pro_mode_failed', 'Pro mode failed');
		}

		const id = crypto.randomUUID();
		let comfyPromptId: string;
		try {
			comfyPromptId = await submitProMode(
				platform,
				{
					image: media.get(parsed.data.imageKey)!.url,
					referenceImage: parsed.data.referenceImageKey
						? media.get(parsed.data.referenceImageKey)!.url
						: undefined,
					prompt: parsed.data.prompt,
					speedVsQuality: parsed.data.speedVsQuality
				},
				url.origin,
				id
			);
			...
			if (error instanceof ComfyUiError) {
				...
				if (error.code === 'invalid_configuration') {
					logFailure(500, 'pro_mode_failed', detail);
					return apiError(500, 'pro_mode_failed', 'Pro mode failed');
				}
				logFailure(502, 'pro_mode_failed', detail);
			} else {
				logFailure(502, 'pro_mode_failed', { operation: 'provider_submission' });
			}
			return apiError(502, 'pro_mode_failed', 'Pro mode failed');
		}
		...
		try {
			...
			logFailure(500, 'pro_mode_failed', { operation: 'job_persistence' });
			try {
				await cancelProMode(platform, comfyPromptId);
			} catch (cleanupError) {
				logFailure(
					500,
					'pro_mode_failed',
		...
		return json({ id, status: 'processing' } satisfies ProModeJobResponse, {
			status: 202,
			headers: {
				'cache-control': 'no-store',
				location: `/api/pro-mode/${id}`
			}
		});
	} finally {
		proModeInFlight.delete(pubkey);
	}
};
