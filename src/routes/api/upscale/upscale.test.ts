...
describe('POST /api/upscale — billing', () => {
	...
	it('records the upscaled image against the authenticated profile', async () => {
		const db = makeD1();
		seedUser(db, 'user-1', 'pubkey-1');
	...
	it('returns 500 if recording the upscaled image fails', async () => {
		const db = makeD1();
		seedUser(db, 'user-1', 'pubkey-1');
		...
		try {
			...
			expect(consoleError).toHaveBeenCalledWith(
				'recordGeneratedImage failed after a successful upscale:',
				expect.any(Error)
			);
		} finally {
