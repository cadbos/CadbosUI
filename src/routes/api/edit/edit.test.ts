...
describe('POST /api/edit — billing', () => {
	...
	it('records source_hash from imageHash when editing a fresh upload with no prior render', async () => {
		...
		const row = await db
			.prepare(
				'SELECT detail.input_hash FROM image_generation_details detail ' +
					'JOIN generations generation ON generation.id = detail.generation_id ' +
					'WHERE generation.user_id = ?'
			)
			.bind('user-1')
	...
	it('returns 500 and retains a confirmed operation when finalization keeps failing', async () => {
		...
		try {
			...
			expect(consoleError).toHaveBeenCalledWith('edit operation failed:', expect.any(Error));
			expect(
				db
