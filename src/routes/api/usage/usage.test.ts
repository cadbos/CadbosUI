...
function seedDeposit(
	...
	const transactionId = `deposit:${depositId}`;
	const creditUnits = toLedgerAmountUnits(creditsAwarded);
	const ledgerCreditUnits = toLedgerAmountUnits(ledgerCredits);
	const archaiUnits = toLedgerAmountUnits(100);
	db.prepare(
		'INSERT INTO packages ' +
			'(id, usd_amount_cents, credits_awarded_units, archai_tokens_awarded_units, enabled, created_at) ' +
			'VALUES (?, 1000, ?, ?, 1, ?)'
	)
		.bind(packageId, creditUnits, archaiUnits, createdAt)
		.run();
	if (status === 'paid') {
		db.prepare('INSERT INTO ledger_transactions (id, occurred_at) VALUES (?, ?)')
			.bind(transactionId, createdAt)
		...
		db.prepare(
			...
			.bind(transactionId, userId, ledgerCreditUnits)
			.run();
		db.prepare(
			'INSERT INTO ledger_entries (transaction_id, account_id, amount) ' +
				"VALUES (?, 'app-credit:system', ?)"
		)
			.bind(transactionId, -ledgerCreditUnits)
			.run();
		db.prepare(
			'INSERT INTO ledger_entries (transaction_id, account_id, amount) ' +
				"VALUES (?, 'archai-token', ?)"
		)
			.bind(transactionId, archaiUnits)
			.run();
		db.prepare(
			'INSERT INTO ledger_entries (transaction_id, account_id, amount) ' +
				"VALUES (?, 'archai-token:system', ?)"
	...
	db.prepare(
		'INSERT INTO deposits ' +
			'(id, request_id, user_id, package_id, provider, provider_checking_id, payment_hash, bolt11, ' +
			'sats_amount, usd_amount_cents, sats_per_usd_rate, credits_awarded_units, ' +
			'archai_tokens_awarded_units, status, created_at, expires_at, paid_at, reconcile_after, ' +
			'ledger_transaction_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1000, 1000, 100, ?, ?, ?, ?, ?, ?, ?, ?)'
	)
		.bind(
			depositId,
			requestId,
			...
			status === 'paid' ? null : createdAt + 30_000,
