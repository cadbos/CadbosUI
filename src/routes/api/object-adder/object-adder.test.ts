...
import type { ObjectAdderJobResponse, SessionUser } from '$lib/api/contract';
import { ComfyUiError, type ComfyDownloadedImage } from '$lib/server/comfyui';
import { DEMO_PUBKEY } from '$lib/server/demo';
import { createObjectAdderJob, getObjectAdderJob } from '$lib/server/object-adder-jobs';
...
vi.mock('$lib/server/object-adder', async (importOriginal) => {
	const actual = await importOriginal<typeof import('$lib/server/object-adder')>();
	return {
		...actual,
		cancelObjectAdder: integration.cancel,
		objectAdderCost: vi.fn(() => {
			if (integration.costError) throw integration.costError;
			return integration.cost;
		}),
		pollObjectAdder: integration.poll,
		submitObjectAdder: integration.submit
	};
});

vi.mock('$lib/server/object-adder-jobs', async (importOriginal) => {
	const actual = await importOriginal<typeof import('$lib/server/object-adder-jobs')>();
	return {
		...actual,
		createObjectAdderJob: vi.fn((...args: Parameters<typeof actual.createObjectAdderJob>) => {
			if (jobStore.failNextCreate) {
				jobStore.failNextCreate = false;
				return Promise.reject(new Error('private persistence detail'));
			}
			return actual.createObjectAdderJob(...args);
		}),
		getObjectAdderJob: vi.fn(actual.getObjectAdderJob)
	};
});
...
const requestBody = {
	...
	objectImage: 'https://cdn.example.test/object.jpg',
...
function callPost(
	...
	return POST({
		request: new Request('https://cadbos.example/api/object-adder', {
			method: 'POST',
			body: JSON.stringify(body)
		}),
		platform: requestPlatform,
		locals: { sessionLookupUnavailable, user },
		url: new URL('https://cadbos.example/api/object-adder')
	} as PostEvent);
}
...
async function seedJob(db: D1Database, createdAt = Date.now()): Promise<void> {
	...
	await createObjectAdderJob(db, {
		...
		objectUrl: requestBody.objectImage,
		objectHash: 'hash-object',
		rect: requestBody.rect,
...
describe('POST /api/object-adder', () => {
	it('requires authentication and a strict request body', async () => {
		...
		expect(createObjectAdderJob).not.toHaveBeenCalled();
	...
	it('logs billing pre-check failures without exception details', async () => {
		...
		expect(await response.json()).toEqual({
			error: { code: 'object_adder_failed', message: 'Object adder failed' }
		});
		expectSingleLog(
			consoleError.mock.calls.flat(),
			failureLog(500, 'object_adder_failed', 'billing_precheck')
		);
	...
	it('submits once and returns a persisted polling resource', async () => {
		...
		const response = await callPost({ pubkey: 'pubkey-1' }, platform(db));
		const result = (await response.json()) as ObjectAdderJobResponse;

		expect(response.status).toBe(202);
		expect(response.headers.get('location')).toBe(`/api/object-adder/${id}`);
		...
		await expect(getObjectAdderJob(db, 'user-1', id)).resolves.toMatchObject({
			comfyPromptId: 'prompt-1',
			cost: 3.5,
			status: 'processing'
		});
	...
	it.each([
		...
		async (code, expectedStatus, providerStatus) => {
			...
			expectSingleLog(
				consoleError.mock.calls.flat(),
				failureLog(expectedStatus, 'object_adder_failed', 'provider_submission', {
					providerCode: code,
			...
			const count = await db
				.prepare('SELECT COUNT(*) AS count FROM object_adder_jobs')
				.first<{ count: number }>();
	...
	it('maps a composite failure to a 415 unsupported_image_type response', async () => {
		const db = makeD1();
		seedUser(db, 12);
		integration.submit.mockRejectedValue(new ObjectAdderCompositeError('Unsupported image format'));
		const consoleError = vi.spyOn(console, 'error').mockImplementation(() => undefined);
		...
		expectSingleLog(
			consoleError.mock.calls.flat(),
			failureLog(415, 'unsupported_image_type', 'composite')
		);
		const count = await db
			.prepare('SELECT COUNT(*) AS count FROM object_adder_jobs')
			.first<{ count: number }>();
	...
	it('logs unknown submission failures without exception details', async () => {
		...
		expectSingleLog(
			consoleError.mock.calls.flat(),
			failureLog(502, 'object_adder_failed', 'provider_submission')
		);
	...
	it('cancels the accepted prompt and logs job persistence failures without stored request data', async () => {
		...
		expectSingleLog(
			consoleError.mock.calls.flat(),
			failureLog(500, 'object_adder_failed', 'job_persistence')
		);
		const logged = consoleError.mock.calls.flat().join(' ');
		expect(logged).not.toContain(requestBody.image);
		expect(logged).not.toContain(requestBody.objectImage);
	...
	it('logs a sanitized failure when persistence cleanup also fails', async () => {
		...
		expect(consoleError.mock.calls.flat()).toEqual([
			JSON.stringify(failureLog(500, 'object_adder_failed', 'job_persistence')),
			JSON.stringify(
				failureLog(500, 'object_adder_failed', 'job_persistence_cleanup', {
	...
	it('rejects a concurrent submission for the same account and isolates the guard by pubkey', async () => {
		...
		expect(await duplicate.json()).toEqual({
			error: {
				...
				message: 'Object adder request already in progress'
			}
		});
...
describe('GET /api/object-adder/[id]', () => {
	it('returns 503 without loading, polling, or storing a job when session lookup is unavailable', async () => {
		...
		expect(getObjectAdderJob).not.toHaveBeenCalled();
	...
	it('finalizes and charges a completed result exactly once across concurrent polls', async () => {
		...
		)) as ObjectAdderJobResponse[];

		expect(results).toEqual([
			{
				id: 'job-1',
				status: 'completed',
				outputUrl: 'https://cdn.example.test/object-adders/job-1.png',
				cost: 2,
				balance: 10
			},
			{
	...
	it('times out only after checking ComfyUI for a completed result', async () => {
		...
		expect(result).toEqual({
			...
			status: 'failed',
			error: {
				code: 'object_adder_timeout',
				message: 'Object adder timed out'
			}
		});
	});
});
