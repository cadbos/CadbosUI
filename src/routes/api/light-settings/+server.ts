...
import { apiError, lightSettingsRequestSchema, parseBody } from '$lib/server/api';
...
const LIGHT_SETTINGS_RATE_LIMIT = { windowMs: 60_000, max: 10 } as const;
const lightSettingsInFlight = new Set<string>();
...
export const POST: RequestHandler = async ({ request, platform, locals, url }) => {
	...
	const parsed = await parseBody(request, lightSettingsRequestSchema);
	...
	// that window (mirrors object-replacement's/texture-replacement's guard).
	const pubkey = locals.user.pubkey;
	if (lightSettingsInFlight.has(pubkey)) {
		logRejection(409, 'request_in_progress');
		return apiError(409, 'request_in_progress', 'Light settings request already in progress');
	}
	lightSettingsInFlight.add(pubkey);
	...
	try {
		...
		const limited = await touchRateLimit(
			db,
			`light-settings:${pubkey}`,
			Date.now(),
			LIGHT_SETTINGS_RATE_LIMIT
		);
		if (limited) {
		...
		let cost: number;
		try {
			cost = lightSettingsCost(platform);
			...
			logFailure(500, 'light_settings_failed', { operation: 'billing_precheck' });
			return apiError(500, 'light_settings_failed', 'Light settings failed');
		}
		...
		try {
			...
			if (error instanceof ComfyUiError) {
				...
				if (error.code === 'invalid_configuration') {
					logFailure(500, 'light_settings_failed', detail);
					return apiError(500, 'light_settings_failed', 'Light settings failed');
				}
				logFailure(502, 'light_settings_failed', detail);
			} else {
				logFailure(502, 'light_settings_failed', { operation: 'provider_submission' });
			}
		...
		try {
			...
			try {
				await cancelLightSettings(platform, comfyPromptId);
			} catch (cleanupError) {
				logFailure(
					500,
					'light_settings_failed',
		...
		return json({ id, status: 'processing' } satisfies LightSettingsJobResponse, {
			status: 202,
			headers: {
				'cache-control': 'no-store',
				location: `/api/light-settings/${id}`
			}
		});
	} finally {
		lightSettingsInFlight.delete(pubkey);
	}
};
