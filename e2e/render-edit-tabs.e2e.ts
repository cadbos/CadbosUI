...
async function uploadImageFile(
	...
	await expect(async () => {
		...
		await expect(input.locator('..').getByRole('button', { name: 'Изменить фото' })).toBeVisible({
			timeout: 2_000
		});
	}).toPass({ timeout: 10_000 });
}

async function generateBase(page: Page): Promise<void> {
...
async function mockBaseRender(page: Page): Promise<void> {
...
test('a failed edit request surfaces the error and retains the current result', async ({
	page
}) => {
	await authenticate(page);
	await mockUpload(page);
...
test('the result toolbar supports undo/redo, comparing before/after, and upscaling to 4K', async ({
	...
	const undoButton = page.getByRole('button', { name: 'Отменить последнюю правку' });
	const redoButton = page.getByRole('button', { name: 'Вернуть отменённую правку' });
