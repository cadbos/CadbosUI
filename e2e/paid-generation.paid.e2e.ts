...
test('render, edit, and upscale chain through durable local billing', async ({ page, context }) => {
	...
	const editSummary = editPanel.locator('.edit-panel > .meta');
	await expect(editSummary.getByText('Стоимость правки: 2.00', { exact: true })).toBeVisible();
	await expect(editSummary.getByText('Баланс: 16.00', { exact: true })).toBeVisible();
	...
	await expect(editSummary.getByText('Стоимость правки: 3.00', { exact: true })).toBeVisible();
	await expect(editSummary.getByText('Баланс: 13.00', { exact: true })).toBeVisible();

	const platform = await getPlatformProxy<{ DB: D1Database }>({
		configPath: './wrangler.jsonc',
		persist: { path: PAID_FLOW_PERSIST_PATH },
